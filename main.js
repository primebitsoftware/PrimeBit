...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const DEFAULT_WHATSAPP = '+962 79 825 9958';
    ...
    if (orderBtn) {
        orderBtn.addEventListener('click', () => {
            ...
            saveToSupabase('primebit_project_leads', {
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', async (e) => {
            ...
            try {
                await saveToSupabase('primebit_contact_messages', { name, email, phone, subject, message, lang: currentLang });
                cloudSaved = true;
            } catch (err) {
    ...
    function runTerminal(name, email, phone, subject, emailSent) {
        ...
        const lines = [
            { text: "$ sh primebit_transmit_inquiry.sh", type: "input", delay: 100 },
            { text: ar ? "[*] تهيئة بروتوكول الإرسال الآمن..." : "[*] Initializing PRIMEBIT Secure Protocol...", type: "system", delay: 500 },
            ...
            { text: ar ? "$ شكراً لاختيارك PRIMEBIT." : "$ Thank you for choosing PRIMEBIT.", type: "input", delay: 300 }
        ];
